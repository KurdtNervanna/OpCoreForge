...
from pt_scripts import plist, plistwindow, downloader

def _check_for_update(queue, version_url = None, user_initiated = False):
    args = [sys.executable]
    file_path = os.path.join(os.path.abspath(os.path.dirname(__file__)),"pt_scripts","update_check.py")
...
class ProperTree:
    def __init__(self, plists = []):
        ...
        self.settings = {}
        if os.path.exists("pt_scripts/settings.json"):
            try:
                self.settings = json.load(open("pt_scripts/settings.json"))
            except:
                pass
        # Also load the snapshot defaults
        self.snapshot_data = {}
        if os.path.exists("pt_scripts/snapshot.plist"):
            try:
                with open("pt_scripts/snapshot.plist","rb") as f:
                    self.snapshot_data = plist.load(f)
            except:
                pass
        # And finally, load the version.json if it exists
        self.version = {}
        if os.path.exists("pt_scripts/version.json"):
            try: self.version = json.load(open("pt_scripts/version.json"))
            except: pass
        os.chdir(cwd)
    ...
    def quit(self, event_or_signum=None, frame=None):
        ...
        os.chdir(os.path.abspath(os.path.dirname(__file__)))
        try:
            json.dump(self.settings,open("pt_scripts/settings.json","w"),indent=4)
        except:
            pass
        os.chdir(cwd)
